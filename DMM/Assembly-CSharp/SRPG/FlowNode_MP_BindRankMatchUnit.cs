// Decompiled with JetBrains decompiler
// Type: SRPG.FlowNode_MP_BindRankMatchUnit
// Assembly: Assembly-CSharp, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: DAD16919-792A-4B52-A355-9FC67E79AC85
// Assembly location: C:\r\Tagatame-Datamine\DMM\Il2CppDumper\DummyDll\Assembly-CSharp.dll

using Il2CppDummyDll;
using UnityEngine;

#nullable disable
namespace SRPG
{
  [Token(Token = "0x20017CA")]
  [FlowNode.NodeType("MultiPlay/RankMatch/BindRankMatchUnit", 32741)]
  [FlowNode.Pin(1, "Set", FlowNode.PinTypes.Input, 1)]
  [FlowNode.Pin(11, "Out", FlowNode.PinTypes.Output, 11)]
  [AddComponentMenu("")]
  public class FlowNode_MP_BindRankMatchUnit : FlowNode
  {
    [Token(Token = "0x400558D")]
    private const int PIN_IN_SET = 1;
    [Token(Token = "0x400558E")]
    private const int PIN_OUT_SET = 11;
    [Token(Token = "0x400558F")]
    [FieldOffset(Offset = "0x18")]
    [FlowNode.DropTarget(typeof (GameObject[]), false)]
    [SerializeField]
    private GameObject[] Targets;
    [Token(Token = "0x4005590")]
    [FieldOffset(Offset = "0x1C")]
    [SerializeField]
    private FlowNode_MP_BindRankMatchUnit.TargetType Type;

    [Token(Token = "0x600605F")]
    [Address(RVA = "0x1248FA0", Offset = "0x1247DA0", VA = "0x11248FA0")]
    private void Out()
    {
    }

    [Token(Token = "0x6006060")]
    [Address(RVA = "0x134AD20", Offset = "0x1349B20", VA = "0x1134AD20", Slot = "4")]
    public override void OnActivate(int pinID)
    {
    }

    [Token(Token = "0x6006061")]
    [Address(RVA = "0x306200", Offset = "0x305000", VA = "0x10306200")]
    public FlowNode_MP_BindRankMatchUnit()
    {
    }

    [Token(Token = "0x20017CB")]
    public enum TargetType
    {
      [Token(Token = "0x4005592")] Player,
      [Token(Token = "0x4005593")] Enemy,
    }
  }
}

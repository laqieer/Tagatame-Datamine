// Decompiled with JetBrains decompiler
// Type: SRPG.GenesisQuestConfirm
// Assembly: Assembly-CSharp, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: DAD16919-792A-4B52-A355-9FC67E79AC85
// Assembly location: C:\r\Tagatame-Datamine\DMM\Il2CppDumper\DummyDll\Assembly-CSharp.dll

using Il2CppDummyDll;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;

#nullable disable
namespace SRPG
{
  [Token(Token = "0x20024F0")]
  [AddComponentMenu("UI/Genesis/GenesisQuestConfirm")]
  [FlowNode.Pin(1, "Initialize", FlowNode.PinTypes.Input, 1)]
  public class GenesisQuestConfirm : MonoBehaviour, IFlowInterface
  {
    [Token(Token = "0x400A8A9")]
    public const int PIN_IN_INIT = 1;
    [Token(Token = "0x400A8AA")]
    [FieldOffset(Offset = "0xC")]
    [SerializeField]
    private QuestCampaignList CampaignPrefab;
    [Token(Token = "0x400A8AB")]
    [FieldOffset(Offset = "0x10")]
    [SerializeField]
    private GameObject DetailInfoPrefab;
    [Token(Token = "0x400A8AC")]
    [FieldOffset(Offset = "0x14")]
    [SerializeField]
    private Text Text_StaminaVal;
    [Token(Token = "0x400A8AD")]
    [FieldOffset(Offset = "0x18")]
    [SerializeField]
    private List<GameObject> mRefMissionStarLists;
    [Token(Token = "0x400A8AE")]
    [FieldOffset(Offset = "0x1C")]
    [SerializeField]
    private List<GameObject> mRefDifficultyObject;
    [Token(Token = "0x400A8AF")]
    [FieldOffset(Offset = "0x20")]
    private QuestParam mQuestParam;
    [Token(Token = "0x400A8B0")]
    [FieldOffset(Offset = "0x24")]
    private QuestCampaignData[] mCampaigns;

    [Token(Token = "0x600A22C")]
    [Address(RVA = "0x643EE0", Offset = "0x642CE0", VA = "0x10643EE0")]
    private bool Init() => new bool();

    [Token(Token = "0x600A22D")]
    [Address(RVA = "0x6441F0", Offset = "0x642FF0", VA = "0x106441F0")]
    public void OnOpenItemDetail()
    {
    }

    [Token(Token = "0x600A22E")]
    [Address(RVA = "0x2781C0", Offset = "0x276FC0", VA = "0x102781C0")]
    private void Update()
    {
    }

    [Token(Token = "0x600A22F")]
    [Address(RVA = "0x643E90", Offset = "0x642C90", VA = "0x10643E90", Slot = "4")]
    public void Activated(int pinID)
    {
    }

    [Token(Token = "0x600A230")]
    [Address(RVA = "0x6442E0", Offset = "0x6430E0", VA = "0x106442E0")]
    public void SetMissionStar(QuestParam param)
    {
    }

    [Token(Token = "0x600A231")]
    [Address(RVA = "0x2ACF80", Offset = "0x2ABD80", VA = "0x102ACF80")]
    public GenesisQuestConfirm()
    {
    }
  }
}
